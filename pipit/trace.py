...
class Trace:
    ...
    # def _get_mpi_comm_df(self):
    #     """Returns a dataframe containing only MPI communication events"""
    #     map = ["Timestamp (ns)", "Receieved From Process", "Sent to Process", "Process"]
    #     # df = pd.DataFrame(columns=columns)
    #     send_events = self.events.loc[self.events['Event Type'] == 'Instant'].loc[self.events["Name"].isin(["MpiSend"])]
    #     # match mpi events if they're not already
    #     if send_events['_matching_event'].isnull().all():
    #         self._match_mpi()
        
    #     receive_events = self.events.loc[send_events['_matching_event']]

    #     send_to_add = pd.DataFrame(send_events['Timestamp (ns)'], send_events['Process'], )
